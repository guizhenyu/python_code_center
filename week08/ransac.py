...
def ransc(data, model, n, k, t, d, debug = False, return_all = False):
    iterations = 0
    bestfit = None
    besterr = np.inf
    ...
    while iterations < k:
        ...
        if debug:
            ...
            print ('test_err_max()', test_err.max())
            print ('numpy.mean()', np.mean(test_err))
            print ('iteration %d: len(alsoinliners)=%d' % (iterations, len(also_inliers)))
...
def test():
    ...
    if 1:
        ...
        B_noisy[outlier_idxs] = 50 * np.random.normal(size=(n_inputs,n_outputs))

    all_data = np.hstack((A_noisy, B_noisy))
    input_columns = range(n_inputs)
    output_columns = [n_inputs + y for y in range(n_outputs)]
    debug = False
    model = LinearLeastSquareModel(input_columns,output_columns,debug=debug)

    linear_fit, resids, rank, s = sp.linalg.lstsq(all_data[:,input_columns], all_data[:, output_columns])
    ransac_fit, ransac_data = ransc(all_data, model, 50, 1000, 7e3,300, debug = debug, return_all=True)
    if 1:
        import pylab
